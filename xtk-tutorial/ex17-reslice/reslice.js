window.addEventListener("DOMContentLoaded", () => {
  function sizeContent() {
    function computeLivereSliceDelta() {
      const livereslice = document.querySelector("#livereslice");
      const { height, marginTop, paddingBottom } = window.getComputedStyle(
        livereslice
      );
      const heightDeltas = [height, marginTop, paddingBottom].map(parseFloat);
      return (
        livereslice.offsetTop +
        heightDeltas.reduce((acc, value) => acc + value, 0)
      );
    }

    function computeSummariesDelta() {
      const summaries = document.querySelectorAll("summary");
      const bordersPxDelta = 2;
      return [...summaries].reduce(
        (acc, summary) => acc + bordersPxDelta + summary.offsetHeight,
        0
      );
    }

    function computeJsFiddleDelta() {
      const jsfiddle = document.querySelector("#jsfiddle");
      const defaultPxValue = 75;
      const computedPxValue = jsfiddle.offsetHeight * 2;
      return Math.max(defaultPxValue, computedPxValue);
    }

    function setContentsMaxHeight(maxHeight) {
      const contents = document.querySelectorAll(".content");
      contents.forEach(content => (content.style.maxHeight = `${maxHeight}px`));
    }

    function getDeltasSum() {
      const deltasFunsResults = [
        computeLivereSliceDelta(),
        computeSummariesDelta(),
        computeJsFiddleDelta()
      ];
      const deltasSum = deltasFunsResults.reduce(
        (acc, delta) => acc + delta,
        0
      );
    ...
    const maxHeight = window.innerHeight - getDeltasSum();
    setContentsMaxHeight(maxHeight);
  }
  ...
  xslicegui.prototype.setupslicegui = function() {
    ...
    this.sliceXNXController = this.slicegui
      .add(this.volume, "xNormX", -1, 1)
      .name("Normal X Dir.")
      .listen();
    this.sliceXNYController = this.slicegui
      .add(this.volume, "xNormY", -1, 1)
      .name("Normal Y Dir.")
      .listen();
    this.sliceXNZController = this.slicegui
      .add(this.volume, "xNormZ", -1, 1)
      .name("Normal Z Dir.")
      .listen();
    this.sliceXNCController = this.slicegui
      .addColor(this, "color")
      .name("Color")
      .listen();
    this.slicegui.open();
    ...
    normalChange = () => {
      _this.color = [
        Math.abs(_this.volume.xNormZ),
        Math.abs(_this.volume.xNormY),
        Math.abs(_this.volume.xNormX)
      ];
      if (_this.coloring) {
    ...
    this.sliceXNCController.onChange(() => {
      if (_this.coloring) {
        _this.volume.xColor = _this.color;
        _this.volume.maxColor = _this.volume.xColor;
        _this.volume.sliceInfoChanged(0);
  ...
  xslicegui.prototype.setupnavgui = function() {
    ...
    this.sliceXController = this.navgui
      .add(this.volume, "indexX", 0, this.volume.range[0] - 1)
      .name("Slice Index")
      .listen();
    this.navgui.open();

    const _this = this;
    this.sliceXController.onChange(() => {
      // Hide Y and Z slices
  ...
  xslicegui.prototype.setupscenegui = function() {
    ...
    this.sceneOrientationController = this.scenegui
      .add(this, "sceneOrientation", {
        Free: 0,
        Sagittal: 1,
        Coronal: 2,
        Axial: 3
    ...
    // callbacks
    this.renderer.interactor.addEventListener(
      X.event.events.ROTATE,
      this.updateSceneView
    );

    const _this = this;
    this.sceneOrientationController.onChange(value => {
      if (value == 1) {
        // move camera
  ...
  xslicegui.prototype.updateSceneView = function() {
    ...
    if (_this.sliceMode.getValue() == 1 || _this.sliceMode.getValue() == 4) {
      const _x = _this.renderer.camera.view[2];
      const _y = _this.renderer.camera.view[6];
      const _z = _this.renderer.camera.view[10];
      // normalize
      const length = Math.sqrt(_x * _x + _y * _y + _z * _z);
  ...
  xslicegui.prototype.setupmodegui = function() {
    ...
    this.coloringMode = this.modegui
      .add(this, "coloring")
      .name("Slice Coloring");
    this.modegui.open();
    ...
    this.sliceMode.onChange(value => {
      if (value == 0) {
        ...
        // setup demo
        const _this2 = _this;
        _this.demoIntervalID = setInterval(() => {
          _this2.reslice();
          _this2.volume.sliceInfoChanged(0);
          _this2.sliceXController.__max = _this2.volume.range[0] - 1;
        }, 5);
      } else if (value == 1) {
        // cleanup demo
        ...
        _this.lmController.on("connect", function() {
          _this.lmIntervalID = setInterval(() => {
            const frame = _this.lmController.frame();
            const handString = "";
            if (frame.hands.length > 0) {
              ...
              _this.volume.sliceInfoChanged(0);
              _this.sliceXController.__max = _this.volume.range[0] - 1;
              _this.renderer.camera.position = [
                -500 * _this.volume.xNormX,
                -500 * _this.volume.xNormY,
                -500 * _this.volume.xNormZ
              ];
            }
          }, 15);
        });
        ...
        _this.lmController.on("connect", function() {
          _this.lmIntervalID = setInterval(() => {
            ...
            if (frame.pointables.length > 0) {
              const finger = frame.pointables[0];

              //4 fingers scroll
              if (frame.pointables.length == 4) {
                scroller(
                  finger.tipPosition[0] / speedfactor,
                  (heightMax / 2 - finger.tipPosition[1]) / speedfactor
                );
        ...
        const slider = function(xDir, yDir) {
          _this.lmController.frame();
          _this.renderer.camera.rotate([-xDir, -yDir]);
          _this.updateSceneView();
        };

        const scroller = function(xDir, yDir) {
    ...
    this.bboxMode.onChange(value => (_this.box.visible = value));

    this.coloringMode.onChange(value => {
      if (value) {
        _this.volume.xColor = _this.color;
        _this.volume.maxColor = _this.volume.xColor;
      } else {
  ...
  xslicegui.prototype.reslice = function() {
    ...
    this.color = [
      Math.abs(this.volume.xNormZ),
      Math.abs(this.volume.xNormY),
      Math.abs(this.volume.xNormX)
    ];
    if (this.coloring) {
      this.volume.xColor = [
        Math.abs(this.volume.xNormZ),
  ...
  function initialize() {
    ...
    r.onShowtime = () => {
      ...
      const res = [volume.bbox[0], volume.bbox[2], volume.bbox[4]];
      const res2 = [volume.bbox[1], volume.bbox[3], volume.bbox[5]];
